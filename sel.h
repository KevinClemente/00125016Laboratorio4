void createLocalA(Matrix &A,mesh m){
    ...
    A.at(0).at(0) += -J/8;  A.at(0).at(1) += J/8;
    A.at(1).at(0) += -J/8;  A.at(1).at(1) += J/8;
}

void createLocalC(Matrix &C,mesh m){
    float ALFA = m.getParameter(VAR_ALPHA);
...
void createLocalG(Matrix &G,mesh m){
    float LAMBDA = m.getParameter(VAR_LAMBDA);
    G.at(0).at(0) += -LAMBDA/3;  G.at(0).at(1) += LAMBDA/3;
    G.at(1).at(0) += -LAMBDA/3;  G.at(1).at(1) += LAMBDA/3;
}

void createLocalI(Matrix &I,mesh m){
    float N = m.getParameter(VAR_N);
    I.at(0).at(0) += -N/2.0;  I.at(0).at(1) += N/2.0;
    I.at(1).at(0) += -N/2.0;  I.at(1).at(1) += N/2.0;
}
...
void createLocalQ(Matrix &Q,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float L = m.getParameter(VAR_L);
    Q.at(0).at(0) += L/l;      Q.at(0).at(1) += -L/l;
    Q.at(1).at(0) += -L/l;     Q.at(1).at(1) += L/l;
}
...
Vector createLocalb(int element,mesh &m){
    ...
    float M = m.getParameter(VAR_M), P = m.getParameter(VAR_P), l = m.getParameter(ELEMENT_LENGTH);
